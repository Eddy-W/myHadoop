...
public class DSPCategoryDecode_ext extends UDF {
	...
	DSPCategoryDecode_ext() throws IOException{
		buildAdxTree();
	}
	...
	public String evaluate(Object... args) throws IOException {
        ...
        if(argsNum==3){
        	...
        	int pl = Integer.parseInt(columnValue[2].toString());
        	if(type.equals("id")){
        		String[] nativecodes = nativecodestr.split(",");
        		for(int i=0;i<nativecodes.length;++i){
        			treeNode<String> tmp= this.AdxTree.search2level(nativecodes[i], pl);
        			if(tmp!=null)
        			{
        				res+= tmp.ID.toString()+",";
        			}
	...
	private boolean checkCategory(treeNode<String> node,String checkstr) {
		// TODO Auto-generated method stub
		String[] tmp = checkstr.split(",");
		for (int i=0;i<tmp.length;++i){
			if(node.ID.toString().equals(tmp[i]))
				return true;
		}
		return false;
	}
    ...
    public static void main(String[] args) throws IOException{
    	DSPCategoryDecode_ext dd = new DSPCategoryDecode_ext();
    	System.out.println(dd.evaluate("3,23,1591,1439","name",4));
    	 
    	System.out.println(dd.evaluate("1137,22,1177,693,1466","name",1)); 
    }
}
